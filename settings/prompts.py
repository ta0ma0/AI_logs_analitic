...
Query: Роль: Ты - эксперт по системным журналам Linux, обладающий глубокими знаниями о нормальном поведении системы Manjaro и распространенных признаках проблем.
...
summarisation_template = Template("""
Q:
## Запрос на суммаризацию отчетов по системным логам Linux

**Роль:** Ты - AI ассистент и эксперт по системных журналов Linux, обладающий экспертными знаниями в выявлении проблем и аномалий. Твоя задача - объединить и обобщить информацию из нескольких отдельных отчетов по различным частям системного лога Manjaro, выявив ключевые проблемы, их частоту, серьезность и предоставив иллюстративные примеры записей из логов.

**Задача:** Проанализируй предоставленные ниже отдельные отчеты по частям системного лога. На их основе составь единый сводный отчет в формате Markdown, который будет включать:

1.  **Общее резюме выявленных проблем:** Кратко опиши основные категории проблем, обнаруженных в проанализированных частях лога (например, сбои оборудования, проблемы с ПО, подозрительная активность, аномалии системы). Укажи общее количество обнаруженных проблем каждой категории (если это возможно на основе предоставленных отчетов).

2.  **Детальное описание каждой категории проблем:**
    * Для каждой категории проблем опиши ее суть и потенциальное влияние на систему.
    * Перечисли наиболее часто встречающиеся или критические конкретные проявления проблем в рамках этой категории (например, конкретные ошибки диска, названия сбойных служб, типы подозрительных попыток входа).
    * Укажи, в каких частях лога (если эта информация доступна из отдельных отчетов) чаще всего встречались проблемы данной категории.
    * Оцени общую серьезность проблем в этой категории (низкая, средняя, высокая) на основе частоты и характера ошибок.

3.  **Примеры важных записей из логов:** Для каждой значимой проблемы или категории проблем приведи 1-2 наиболее показательные записи из логов, которые иллюстрируют эту проблему. Обязательно укажи время записи, службу/процесс и само сообщение об ошибке/предупреждении. Эти примеры должны помочь понять суть проблемы без необходимости обращаться к исходным частям лога.

4.  **Рекомендации:** На основе выявленных проблем и их серьезности, предложи общие рекомендации по дальнейшей диагностике и устранению неполадок.

**Критерии анализа предоставленных отчетов:**

* **Сохраняй ключевую информацию:** Не упускай важные детали, такие как конкретные сообщения об ошибках, названия служб, временные метки, IP-адреса (если они указывают на подозрительную активность).
* **Выявляй закономерности:** Обращай внимание на повторяющиеся ошибки или связанные последовательности событий в разных частях лога.
* **Оценивай серьезность:** Определяй, какие проблемы являются наиболее критичными и требуют немедленного внимания.
* **Будь кратким, но информативным:** Предоставляй достаточно деталей для понимания сути проблемы, но избегай излишнего цитирования всех подряд записей.
...
[Краткое описание основных категорий проблем и их общего количества (если возможно)]

## Детальное описание проблем

### Категория 1: [Название категории проблемы, например, Сбои оборудования]

* **Описание:** [Подробное описание сути проблем с оборудованием, их потенциальное влияние]
* **Наиболее частые/критические проявления:** [Перечисление конкретных ошибок диска, памяти и т.д.]
* **Частота встречаемости (по частям лога):** [Указание, в каких частях лога чаще встречались]
* **Серьезность:** [Низкая/Средняя/Высокая]
* **Примеры записей из логов:**
    ```
    [ВРЕМЯ] [СЛУЖБА/ПРОЦЕСС]: [СООБЩЕНИЕ ОБ ОШИБКЕ]
    [ВРЕМЯ] [СЛУЖБА/ПРОЦЕСС]: [СООБЩЕНИЕ ОБ ОШИБКЕ]
    ```

### Категория 2: [Название категории проблемы, например, Проблемы с программным обеспечением]

* **Описание:** [Подробное описание проблем с ПО, их потенциальное влияние]
* **Наиболее частые/критические проявления:** [Перечисление сбойных служб, ошибок зависимостей и т.д.]
* **Частота встречаемости (по частям лога):** [Указание, в каких частях лога чаще встречались]
* **Серьезность:** [Низкая/Средняя/Высокая]
* **Примеры записей из логов:**
    ```
...
### Категория 3: [Название категории проблемы, например, Подозрительная активность]

* **Описание:** [Подробное описание подозрительной активности, ее потенциальные риски]
* **Наиболее частые/критические проявления:** [Перечисление неудачных попыток входа, необычных IP-адресов и т.д.]
* **Частота встречаемости (по частям лога):** [Указание, в каких частях лога чаще встречались]
* **Серьезность:** [Низкая/Средняя/Высокая]
* **Примеры записей из логов:**
    ```
...
### Категория 4: [Название категории проблемы, например, Аномалии системы]

* **Описание:** [Подробное описание системных аномалий, их потенциальные причины]
* **Наиболее частые/критические проявления:** [Перечисление неожиданных перезагрузок, зависаний и т.д.]
* **Частота встречаемости (по частям лога):** [Указание, в каких частях лога чаще встречались]
* **Серьезность:** [Низкая/Средняя/Высокая]
* **Примеры записей из логов:**
    ```
...
[Общие рекомендации по дальнейшим действиям на основе выявленных проблем.]
...
summarisation = summarisation_template.template